...
whether
it
's possible to divide this array into k non-empty subsets whose sums are all equal.
...
Output: True
Explanation: It
's possible to divide it into 4 subsets (5), (1, 4), (2,3), (2,3) with equal sums.
Note:
...
each
number in nums, we
'll check whether putting it in the i-th group solves the problem. We can check those possibilities by recursively searching.
...
of
k
group - sums, as long as the
group
's sum would not exceed the target. For each of these choices, we recursively search with one less number to consider in nums. If we placed every number successfully, then our search was successful.
...
array
groups, by
enforcing if (groups[i] == 0)
break;.This
greatly
reduces
repeated
work -
for example, in the first run of search, we will make only 1 recursive call, instead of k.Actually, we could do better by skipping any repeated values of groups[i], but it isn't necessary.
...
length
of
nums, and kk is as given.As
we
skip
additional
zeroes in groups, naively
we
will
make
O(k!)O(k!) calls
...
one
number.If
for example our target is 10, and our elements to be placed in order are[6, 5, 5, 4], this would not work as 6 + 5 "crosses" 10 prematurely.
